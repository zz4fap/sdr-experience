...
class PfaVsNoisePowerSimu(gr.top_block):
   ...
   def __init__(self, ssblock):
      ...
      self.rtlsdr_source.set_center_freq(25e6, 0)
...
def simulate_pfa(simu_time):
   ...
   fg = PfaVsNoisePowerSimu(ssblock)
   primary_users = []
   scanned_frequencies = []
   for i in range(87, 108):
      for j in range(0,5):
         ...
         freq = float(i + step)
         scanned_frequencies.append(freq-0.2)
         freq = freq*1e6
         print freq
         fg.setFrequency(freq)
         fg.start()
         sleep(simu_time)
         fg.stop()
         fg.wait()
         ...
         detection_rate = soma/size
         print detection_rate
         if(detection_rate > 0.9):
            primary_users.append(1)
         else:
            primary_users.append(0)
   return scanned_frequencies, primary_users
...
if __name__ == "__main__":
   ...
   simu_time = 0.7
   scanned_frequencies, primary_users = simulate_pfa(simu_time)
