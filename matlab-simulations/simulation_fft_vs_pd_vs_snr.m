...
tx_noise_only = false;
complex_noise = true;
calculate_pfa = false; % False: Pd, True: Pfa
...
fft_info(1).fft_size = 512;
fft_info(1).segments_to_check = [7 8 9 10];

fft_info(2).fft_size = 1024;
fft_info(2).segments_to_check = [14 15 16 17 18 19];

fft_info(3).fft_size = 2048;
fft_info(3).segments_to_check = [28 29 30 32 33 34 35 36 37];

fft_info(4).fft_size = 4096;
fft_info(4).segments_to_check = [55 57 59 63 66 70 72 74];

counter = 0;
pd_vs_snr = zeros(3,length(-30:1:30));

for fftNum=1:1:length(fft_info)
    
    NFFT = fft_info(fftNum).fft_size;
    numOfSegments = NFFT/numSamplesSegment; % Number of energy segments.
    Z = zeros(1,numOfSegments);
    segments_to_check = fft_info(fftNum).segments_to_check;
    ...
    for snr=-30:0.1:30
        ...
        for trial = 1:1:numTrials
            ...
            if(tx_noise_only)
                if(complex_noise)
                    tx_signal = wgn(1,L,noisePower,'complex');
                else
                    tx_signal = wgn(1,L,noisePower);
                end
            else
        ...
        filename = sprintf('fft_%d_pd_vs_snr_%dv2.mat',NFFT,snr);
        save(filename, 'NFFT','snr','detection_rate','false_rejection_rate');
        
        counter = counter + 1;
        pd_vs_snr(:,counter) = [NFFT;snr;detection_rate];
    end
end
