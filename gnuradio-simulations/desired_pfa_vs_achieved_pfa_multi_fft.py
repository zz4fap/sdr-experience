...
def writeResultToFile(pfa_simu, pfa_range, fft_size):
   """ Creates a comma separated file for Matlab importing and reading """
   filename = "desired_pfa_vs_achieved_pfa_gnurad_simu_" + str(fft_size) + ".dat"
   f = open(filename, 'w')
...
if __name__ == "__main__":
   ...
   for fft_size in fft_size_vector:
	   ...
	   pfa_simu = [simulate_pfa(dbW, pfa, pfd, nTrials, fft_size) for pfa in pfa_range]
	   writeResultToFile(pfa_simu, pfa_range, fft_size)
