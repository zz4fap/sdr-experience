...
class PfaVsNoisePowerSimu(gr.top_block):
   ...
   def __init__(self, signal, signal_power, snr, pfa, pfd):
...
def simulate_pd(signal, snr, pfa, pfd, nTrials):
   ...
   for j in range(1,nTrials+1):
      fg = PfaVsNoisePowerSimu(signal, signal_power, snr, pfa, pfd)
      fg.run()
...
def writeResultToFile(pd_simu, pfa_range):
   """ Creates a comma separated file for Matlab importing and reading """
   filename = "pfa_vs_achieved_pd_gnurad_simu.dat"
   f = open(filename, 'w')
...
if __name__ == "__main__":
   ...
   print "Simulating..."
   pd_simu = [simulate_pd(hil_fm, snr, pfa, pfd, nTrials) for pfa in pfa_range]
   writeResultToFile(pd_simu, pfa_range)
